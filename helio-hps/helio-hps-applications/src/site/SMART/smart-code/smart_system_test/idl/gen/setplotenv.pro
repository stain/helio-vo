pro setplotenv, xwin=xwin, psfile=psfile, default=default, filename=filename, $
                xs=xs, ys=ys, sys=sys, _extra=_extra
...
;	!p.psym = 8
	...
	print,[[' '],['Environment variables have been optimized for XWindows.'],[' ']]
endif

if keyword_set(psfile) then begin
        if not keyword_Set(xs) then xs=12
...
;	save,rr,gg,bb,file='tmp_ct_store.sav'
	...
	print,[[' '],['Environment variables have been optimized for PostScript.'],[' ']]
...
;		encapsulate=encapsulate, _extra=_extra
	...
	if keyword_set(filename) then begin & psopen,filename, /times, /bold, /inch, $
		/encapsulate, xs=xs, ys=ys, _extra=_extra, /color & setcolors,nocolors=nocolors,sys=sys & endif


	if keyword_set(filename) then print,[[' '],['PostScript file has been opened.'],[' ']]
endif
