function WL_DetSpGS0, image, nx, ny, xc, yc, Radius, qsint, $
...
if not keyword_set(sbl) then $
	binary=WL_Morphological(image, xc, yc, Radius, threshold=threshold, cent=cent) $
		else binary=WL_Morphological(image, xc, yc, Radius, threshold=threshold, cent=cent, /sbl, edges=edges)
...
for i=0, n-1 do begin
	WL_RegProcess2, image, *plocs[i], moms[i, *], qsint, imageResult, imageResult2
endfor
