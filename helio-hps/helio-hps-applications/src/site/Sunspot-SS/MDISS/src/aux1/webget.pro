...
FUNCTION webget,  url,  SILENT=silent, COPYFILE=copyfile, POST=post, $
  ...
  colon1 = StrPos(strmid(url, 7), ':')    ;Position of first colon
  slash1 = StrPos(strmid(url, 7), '/')    ;Position of first slash
  if (colon1 ne -1) then Server = StrMid(url, 7,colon1 ) else Server = StrMid(url,7,slash1)
  ...
  ENDIF ELSE BEGIN 
     ...
     if (colon1 ne -1) then $
     	Port = long(strmid(url,colon1+8,slash1 - colon1 - 1)) $
     else Port = 80 	
