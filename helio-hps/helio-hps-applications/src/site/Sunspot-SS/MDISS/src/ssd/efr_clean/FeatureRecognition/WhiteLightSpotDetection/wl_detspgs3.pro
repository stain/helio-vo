function WL_DetSpGS3, image, nx, ny, xc, yc, Radius, qsint, $
...
if not keyword_set(sbl) then $
	binary=WL_Morphological(image, xc, yc, Radius, threshold=threshold, cent=cent) $
		else binary=WL_Morphological4(image, xc, yc, Radius, $
			threshold=threshold, cent=cent,  /sbl, edges=edges, show=show)
