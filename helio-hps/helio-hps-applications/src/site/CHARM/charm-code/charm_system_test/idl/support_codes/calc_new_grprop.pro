...
for i=0,1 do begin
 ...
 indexy = [1,3,1,3]
 for j=0,3 do begin  & $
     lonlat = conv_a2h([grstr.CHGR_BR_ARC[indexx[j]],grstr.CHGR_BR_ARC[indexy[j]]],eit_map.time)  & $
     grstr.chgr_br_HG_LONLAT_deg_disk[*,j]=lonlat  & $
     car_lonlat = conv_a2c([grstr.CHGR_BR_ARC[indexx[j]],grstr.CHGR_BR_ARC[indexy[j]]],eit_map.time)  & $
...
;=========================================================================================
	...
	grstr.CHGR_CBR_DEG = conv_a2h([newx,newy],eit_map.time)
	grstr.CHGR_CBR_PIX = [newx_px,newy_px]
	grstr.chgr_cbr_carr_disk = conv_a2c([newx,newy],eit_map.time) 
...
;=========================================================================================
	...
	grstr.CHGR_LON_WIDTH_PX  = grstr.CHGR_BR_PIX[2] - grstr.CHGR_BR_PIX[0]
 	...
 	grstr.CHGR_LON_WIDTH_HG_DEG = $
 		max(grstr.chgr_br_HG_LONLAT_deg_disk[0,*])-min(grstr.chgr_br_HG_LONLAT_deg_disk[0,*])
	grstr.CHGR_LAT_WIDTH_HG_DEG = $
 		max(grstr.chgr_br_HG_LONLAT_deg_disk[1,*])-min(grstr.chgr_br_HG_LONLAT_deg_disk[1,*])
