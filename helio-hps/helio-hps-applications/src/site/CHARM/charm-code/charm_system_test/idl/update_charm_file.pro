...
; Get the EIT file used on the detection
	eit_getfiles,filename=s_f.eit_filename,grid=grid,verbose=verbose,dirname=datadir
	YYYYMMDD = (strsplit(anytim(s_f.date_obs,/ecs),/extract))[0]
...
temp = add_tag(temp,fltarr(2,4),'ch_br_HG_LONLAT_deg_disk',/no_pair)
...
;===========================
   ...
   indexy = [1,3,1,3]
   for j=0,3 do begin  & $
      lonlat = conv_a2h([new_s_ch[i].ch_br_arc[indexx[j]],new_s_ch[i].ch_br_arc[indexy[j]]],eit_map_rot.time)  & $
      new_s_ch[i].ch_br_HG_LONLAT_deg_disk[*,j]=lonlat  & $
      car_lonlat = conv_a2c([new_s_ch[i].ch_br_arc[indexx[j]],new_s_ch[i].ch_br_arc[indexy[j]]],eit_map_rot.time)  & $
   ...
   ;Width
      ...
      new_s_ch[i].CH_LON_WIDTH_HG_DEG = $
         max(new_s_ch[i].ch_br_HG_LONLAT_deg_disk[0,*]) - min(new_s_ch[i].ch_br_HG_LONLAT_deg_disk[0,*])
      new_s_ch[i].CH_LAT_WIDTH_HG_DEG = $
         max(new_s_ch[i].ch_br_HG_LONLAT_deg_disk[1,*]) - min(new_s_ch[i].ch_br_HG_LONLAT_deg_disk[1,*])
...
;=========================================================================================
	...
	new_s_ch[i].chc_latlon_deg_disk = conv_a2h([newx,newy],eit_map_rot.time)
	new_s_ch[i].chc_pix_disk = [newx_px,newy_px]
	new_s_ch[i].chc_carr_disk = conv_a2c([newx,newy],eit_map_rot.time) 
...
temp = add_tag(s_gr,fltarr(2,4),'chgr_br_HG_LONLAT_deg_disk',/no_pair)
