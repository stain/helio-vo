...
public class ArchiveExplorer
{
	...
	public ArchiveExplorer(PathBuilder path) throws MalformedURLException
	{
		super();
		this.path	=	path;
	}
...
//		}
	...
	public String getFirstBefore(Date d)
	{
		...
		else
		{
			 ...
			 */
			while(!pageExists(path.dateToPath(currentDate, currentDepth)))
			{
				System.out.println(path.dateToPath(currentDate, currentDepth));
			...
			try
			{
				LinkedList<String>	content	=	getAllIn(path.dateToPath(currentDate, currentDepth));
