        ...
        <div>
          <table width="100%">
            <tr>
              <td>
                <div class="message"
                  style="margin: 0; clear: both; padding: 0px pt 20pt 5px;">
          ...
          <div id="voTables" style="clear: both; padding: 0px pt 20pt 5px;">
            ...
            <g:form target="_blank" controller="prototype">
              <input id="resultId" name="resultId" type="hidden" value="${responseObject?.result.id}" />
            ...
            <div id="response_save_selection" class="custom_button" style="margin-right: 10px; float: right;">
              Save selection to Data Cart</div>
            <div id="response_download_selection" class="custom_button" style="display: none; margin-right: 10px; float: right;">
              Download selected files/all</div>

            <div style="clear: both; width: 100%"></div>

            <g:if test="${result?.queryInfo?.contains('QUERY_ERROR')}">
              "The server reported an unspecified error. Some context information may be found in the returned VOTable. 
            ...
            <g:each in="${result?.getTables()}" status="x" var="tables">
              ...
              <g:else>
                <table cellpadding="0" cellspacing="0" border="0"
                  class='resultTable' id="resultTable${x}">
                  <thead>
                    <tr>
                      <g:each in="${tables?.getHeaders()}" status="i"
                        var="testInstance">
                        <g:if
                          test="${testInstance == 'url' || testInstance == 'event_detail'}">
                          <g:set var="urls" value="${i}" />
                        </g:if>
                        <th>
                          ${testInstance}
                        </th>
                  ...
                  <tbody>
                    <g:each in="${tables?.getData()}" status="i"
                      var="testInstance">
                      <tr class="${(i % 2) == 0 ? 'gradeB' : 'gradeB'}">
                        <g:each in="${testInstance}" status="j"
                          var="row">
                          <g:if test="${urls == j}">
                            <td><a target="_blank" href="${row}">
                                ${row.substring(row.lastIndexOf('/')+1,row.length())}
                            </a>
                            </td>
                          </g:if>
                          <g:elseif test="${row == '-2147483648'}">
                            <td></td>
                          </g:elseif>
                          <g:else>
                            <td>${row}</td>
