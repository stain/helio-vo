...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;
; PRO LINKEDLIST::DELETE, index, ALL=all, NOFREE=nofree
;
...
;   
;   NOFREE: If this keyword is set ptr_free is used instead of heap_free. Only pointer of
;   list element will be deleted, not every pointer recursively inside the element!
;   Use this option to only remove the element from the list and dont delete it completly.
;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;
...
PRO LINKEDLIST::MOVE_NODE, nodeIndex, location, Before=before,err=err
...
self->Delete, nodeIndex, nofree=nofree
...
PRO LINKEDLIST::DELETE_NODE, index,err=err, nofree=nofree
...
if keyword_set(nofree) eq 0 then begin
  if ptr_valid((*currentNode).item) then heap_free, (*currentNode).item
endif else begin
  if ptr_valid((*currentNode).item) then Ptr_Free, (*currentNode).item
endelse
...
IF index EQ (self.count - 1) THEN begin
 self->Delete_Last_Node, nofree=nofree
 return
ENDIF
...
PRO LINKEDLIST::DELETE_LAST_NODE, nofree=nofree
...
PRO LINKEDLIST::DELETE_NODES, nofree=nofree
...
IF Keyword_Set(all) THEN BEGIN
   self->Delete_Nodes, nofree=nofree
   RETURN
ENDIF
...
self->Delete_Node, index, nofree=nofree
...
PRO LINKEDLIST::ADD_AFTER, item, index, no_copy=no_copy,err=err
...
PRO LINKEDLIST::ADD_BEFORE, item, index,no_copy=no_copy,err=err
...
PRO LINKEDLIST::ADD_TO_END, item,no_copy=no_copy,err=err
...
function linkedlist::get_value,index,no_copy=no_copy,err=err
...
function linkedlist::have_item,index,err=err
...
FUNCTION LINKEDLIST::GET_ITEM, index,err=err
...
FUNCTION LINKEDLIST::GET_NODE, index,err=err
...
PRO LINKEDLIST::HELP, Print=print,err=err
...
FUNCTION LINKEDLIST::INIT, item,no_copy=no_copy
