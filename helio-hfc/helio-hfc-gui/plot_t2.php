...
$filename = "t2_quicklook_".$date.'.png';
...
if (isset($use_sess)) {
	...
	foreach ($tmp['ID_TYPE_II'] as $key=>$id_ar)
	...
	$sql_query = "SELECT t.*, o.*, i.* FROM TYPE_II t, OBSERVATIONS o, OBSERVATORY i WHERE DATE_OBS='$date' ";
	...
	if (count($rs['ID_TYPE_II']) == 0) {
		//$sql_query = "SELECT * FROM VIEW_T3_GUI ";
		$sql_query = "SELECT t.*, o.* FROM TYPE_II t, OBSERVATIONS o, OBSERVATORY i ";
...
require_once ('jpgraph/src/jpgraph_mgraph.php');
	...
	for ($i=0; $i<count($instrume); $i++) {
		...
		if ($with_obs) {
			...
			$trans2 = array("_200_par.csv"=>".png");
			$fits_file = $fits_path.strtr($filename, $trans1);
			$dest_fits = strtr($filename, $trans1);
			$dest_png = strtr($filename, $trans2);
			exec ($global['BIN_PATH']."wget -nv --timeout=5 --tries=1 -P /tmp -nd ".$fits_file);
			if (file_exists('/tmp/'.$dest_fits)) {
				$cmd = "python rabat3_fitstopng.py ".$dest_fits." ".$dest_png;
				exec ($cmd);
				//exec ("rm -f ".'/tmp/'.$dest_fits);
				$cmd = "/usr/bin/convert -normalize /tmp/obs_".$dest_png." /tmp/obs_".$dest_png;
				exec ($cmd);
			}
		}
		...
		if ($with_obs && file_exists('/tmp/obs_'.$dest_png)) $graph[$i]->SetBackgroundImage('/tmp/obs_'.$dest_png, BGIMG_FILLPLOT);
