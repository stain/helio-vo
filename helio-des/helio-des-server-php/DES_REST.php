...
/*  Creation object with data to search results */
	...
	// new request
		      ...
		      $helioRequest->arg[$currentArg] = $this->des_ws->parseCondition($parts[1]); 
	    }		             
	  }
...
/* Generation random jobID (function setID())*/
	    $ID = $this->des_ws->setID(); 
/* Call a function getIPclient() to find client IP and put it in global variable  */ 
            $this->IP = $this->des_ws->getIPclient();
 
/* DD CHECK if HELIO1 has already registerd; if not  DD LOGIN */  
           if ($this->des_ws->ddCheckUser() != 0) {        
	          if ($this->des_ws->ddLogin() != 0) 
                           throw new SoapFault("server00","Server Error: DD Login");  
	          if ($this->ddCheckUser() != 0)  throw new SoapFault("server01","Server Error: Check User");
           }
...
*/
	    $cmd = 'php '.wsDir.'/multiRequest_new.php '.$argument;
	    $pid =  $this->des_ws->background($cmd, $ID);
...
*/
		...
		$pidTimeOut =  $this->des_ws->background($cmdTimeOut, $ID);
             }
             else 
...
// Deleting temporary folders
	    $this->des_ws->rrmdir(resultDir.$ID);
 	  return $ResultInfo;
       }
