...
/**
 * Convenience property descriptor to add restrictions and value domains to a JavaBean property.
 * @param <T> type of the property described by this descriptor.
 * @author MarcoSoldati
 *
 */
public class ConfigurablePropertyDescriptor<T> extends PropertyDescriptor {
    ...
    public ConfigurablePropertyDescriptor(String propertyName,
            Class<?> beanClass, String readMethodName, String writeMethodName)
     ...
     * @return the current configurable property descriptor in order to chain multiple constraints.
     */
     ...
     * @return the current configurable property descriptor in order to chain multiple constraints.
     */
    public <S> ConfigurablePropertyDescriptor<T> setValueDomain(Collection<? extends DomainValueDescriptor<S>> domainValues) {
        @SuppressWarnings("unchecked")
        Collection<DomainValueDescriptor<S>> current = (Collection<DomainValueDescriptor<S>>) this.getValue(VALUE_DOMAIN);  
        if (current != null) {
