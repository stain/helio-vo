...
public class STILUtilsTest {
    /**
     * The stil utils instance to use for this test.
     */
    ...
    @Test
    public void testReadVOElement() throws Exception {
    ...
    public void testConcatenateTables() throws Exception {
        ...
        assertEquals(tables[0].getRowCount() + tables[1].getRowCount(),
                resultInner.getRowCount());
        ...
        assertEquals(tables[0].getRowCount() + tables[1].getRowCount(),
                resultOuter.getRowCount());
        assertEquals(18, resultOuter.getColumnCount());

        // check concatenation of five tables
        StarTable resultFive = stilUtils.concatenate(true, tables[0],
                tables[0], tables[0], tables[1], tables[1]);
        assertEquals(tables[0].getRowCount() * 3 + tables[1].getRowCount() * 2,
                resultFive.getRowCount());
    ...
    /**
     * Tests whether tables can be persisted, retreived again and if cleanup of
     * expired tables works.
     */
    @Test
    public void testPersistence() throws Exception {
        ...
        for (int i = 0; i < tables.length; i++) {
            ...
            assertEquals(tables[i].getColumnCount(),
                    readAgain[i].getColumnCount());

            for (int row = 0; row < tables[i].getRowCount(); row++)
                for (int col = 0; col < tables[i].getColumnCount(); col++)
                    assertEquals(tables[i].getCell(row, col),
                            readAgain[i].getCell(row, col));
        }
        ...
        try {
            ...
            throw new AssertionError(
                    "Persisted file was not purged after it's expiration");
    ...
    public void testHeaderReadout() throws Exception {
        ...
        assertEquals(
                -2147483648,
                tables[0].getColumnInfo(0).getAuxDatumValue(
                        Tables.NULL_VALUE_INFO, Integer.class));

        // can read table header?
        assertEquals("2011-05-02 11:56:16",
                tables[0].getParameterByName("EXECUTED_AT").getValue());

        // can read column description?
        assertEquals("Event identification number (HEC internal number)",
                tables[0].getColumnInfo(0).getDescription());
        ...
        assertEquals(expectedQueryURL1,
                tables[0].getParameterByName("QUERY_URL").getValue());
        ...
        assertEquals(expectedQueryURL2,
                tables[1].getParameterByName("QUERY_URL").getValue());
    }
