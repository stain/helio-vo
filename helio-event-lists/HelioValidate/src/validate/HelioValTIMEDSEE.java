...
public class HelioValTIMEDSEE {
	...
	public static void main(String[] args) {
		...
		String deldir  = "C:\\Development\\HELIO\\Event_lists\\Tests\\timed_see_flare\\";
		
		String valdir  = "C:\\Development\\HELIO\\Event_lists\\Tests\\timed_see_flare\\";
		// input delivered file name
		String delname = deldir + "timed_see_flare.txt";
		
		// input VOTable file name
		String valname = valdir + "timed_see_flare_full.xml";
		...
		try {
			...
			while ((deline = delin.readLine()) != null) {
				...
				if (i>skiplines-1) {
					...
					if (tokens[7].trim().equals("0" + theTDs.getContent(8)) || tokens[7].trim().equals("00" + theTDs.getContent(8))
							|| tokens[7].trim().equals(theTDs.getContent(8)) || tokens[7].trim().equals("NaN")) j = 0;
					...
					if (tokens[9].trim().equals(theTDs.getContent(10)) || tokens[9].trim().equals(theTDs.getContent(10) + "0")) j = 0;
					else System.out.println(tokens[0].trim() + " " + tokens[9].trim() + " " + theTDs.getContent(10));
					
					if (tokens[10].trim().equals(theTDs.getContent(11)) || tokens[10].trim().equals(theTDs.getContent(11) + "0")) j = 0;
